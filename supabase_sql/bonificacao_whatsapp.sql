-- ============================================================
-- Módulo Bonificação WhatsApp — execute este SQL no Supabase
-- (instância externa ezdtulcrqzmgocamjwwl, no SQL Editor)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.consultoras_whatsapp (
  ...
  nome text NOT NULL,
  apelido_canal text,                          -- texto único, ex: "WHATSAPP-ANA"
  point_sale_patterns text[] DEFAULT '{}',     -- valores de tray_orders.point_sale que pertencem a ela
  telefone text,
  meta_individual numeric,                     -- opcional, sobrescreve a meta proporcional
...
CREATE TABLE IF NOT EXISTS public.metas_whatsapp (
  ...
  modo_distribuicao text NOT NULL DEFAULT 'proporcional', -- 'individual' | 'proporcional'
  created_at timestamptz NOT NULL DEFAULT now()
);
...
-- 3. Meta individual por consultora/mês (quando modo = individual)
...
CREATE TABLE IF NOT EXISTS public.config_bonificacao_whatsapp (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  faixas_meta jsonb NOT NULL,        -- [{min_pct, max_pct, bonus_base, label}]
  regras_desconto jsonb NOT NULL,    -- [{max_pct, multiplicador}]
  faixas_ticket jsonb NOT NULL,      -- [{min_valor, acelerador}]
...
-- Seed inicial das regras (executar UMA vez)
...
CREATE TABLE IF NOT EXISTS public.bonus_whatsapp_apurados (
  ...
  status text NOT NULL DEFAULT 'projetado',  -- projetado | aprovado | pago
  data_pagamento date,
  created_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE (mes_referencia, consultora_id)
);
