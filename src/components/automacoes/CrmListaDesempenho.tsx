...
type Campo = "nome" | "periodo" | "pessoas" | "enviados" | "taxa_entrega" | "taxa_leitura" | "taxa_abertura" | "taxa_clique" | "taxa_interacao" | "pedidos" | "receita" | "custo" | "roas" | "conversao" | "custo_por_pedido" | "receita_por_mil";
...
export function CrmListaDesempenho({ tipo, dados, fluxos, carregandoFluxos, onNovo }: Props) {
  ...
  return <div className="space-y-4">
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
      ...
      <CrmKpiCard titulo="Receita" valor={brlCrm(dados?.receita)} icon={HandCoins} /><CrmKpiCard titulo="Custo" valor={brlCrm(dados?.custo)} icon={WalletCards} />
    ...
    <Card>
      <CardHeader className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">{FILTROS.map(([valor, rotulo]) => <Badge key={valor} variant={filtro === valor ? "default" : "outline"} className="cursor-pointer" onClick={() => setFiltro(valor)}>{rotulo}</Badge>)}</div>
      </CardHeader>
