      ...
      <div className="space-y-2">
        ...
        {produtos.map((produto, indice) => (
          <div key={indice} className="flex items-center gap-1">
            <Input className="min-w-0" value={produto} placeholder="retailer_id" onChange={(e) => atualizar({ produtos: produtos.map((p, i) => i === indice ? e.target.value : p) })} />
            <Button size="icon" variant="ghost" disabled={indice === 0} onClick={() => mover(indice, -1)} aria-label="Mover produto para cima"><ChevronUp className="h-4 w-4" /></Button>
            <Button size="icon" variant="ghost" disabled={indice === produtos.length - 1} onClick={() => mover(indice, 1)} aria-label="Mover produto para baixo"><ChevronDown className="h-4 w-4" /></Button>
            <Button size="icon" variant="ghost" onClick={() => atualizar({ produtos: produtos.filter((_, i) => i !== indice) })} aria-label="Remover produto"><Trash2 className="h-4 w-4" /></Button>
          </div>
        ))}
        <Button size="sm" variant="outline" className="w-full" disabled={produtos.length >= 30} onClick={() => atualizar({ produtos: [...produtos, ""] })}><Plus className="mr-2 h-4 w-4" />Adicionar retailer_id</Button>
        <p className="text-[11px] text-muted-foreground">Use o retailer_id do produto. A ordem importa: o primeiro produto define a miniatura.</p>
      </div>
    </div>
  );
}
