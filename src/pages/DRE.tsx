...
// Map grupo_dre from categories to DRE keys
...
// Which keys are "custos variáveis" items
...
function calculateDre(movs: any[], catMap: Record<string, { nome: string; grupoDre: string }>) {
  ...
  movs.forEach((m) => {
    if (m.tipo === "entrada" && m.origem === "bling") {
      ...
      addDetail(details, "receita_vendas", m.descricao ?? "Vendas", m.valor ?? 0);

      // Descontos como dedução
      const desconto = m.valor_desconto ?? 0;
      if (desconto > 0) {
        values["estornos"] += desconto;
        addDetail(details, "estornos", "Descontos Concedidos", desconto);
      }
      ...
      addDetail(details, dreKey, catNome, valor);
    }
  });
...
function addDetail(details: Record<string, Record<string, number>>, key: string, label: string, value: number) {
  if (!details[key]) details[key] = {};
  details[key][label] = (details[key][label] ?? 0) + value;
}
...
export default function DRE() {
  ...
  const catMap = useMemo(() => {
    ...
    (categorias ?? []).forEach((c) => {
      map[c.id] = { nome: c.nome_categoria ?? "Sem categoria", grupoDre: c.grupo_dre ?? "" };
    });
    return map;
  }, [categorias]);
  ...
  // AV% = Análise Vertical (% sobre receita bruta)
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Demonstrativo de Resultado do Exercício — Confecção</p>
        </div>
              ...
              {isLoading ? (
                ...
                <div className="space-y-0">
                  ...
                  {DRE_STRUCTURE.map((row) => {
                    ...
                    const details = dreData.details[row.key] ?? {};
                    const hasDetails = Object.keys(details).length > 0 && row.type === "item";
                    const rb = dreData.values["receita_bruta"] ?? 0;

                    // Determine AV value based on row
                    let avValue = val;
                    if (row.type === "percent") avValue = 0; // percent rows show their own %
                    if (row.type === "header" && row.sign === "-") avValue = val;

                    return (
                      <div key={row.key}>
                        <div className={`flex justify-between items-center px-4 py-2.5 rounded ${getRowStyle(row.type)}`}>
                        ...
                        {hasDetails && Object.keys(details).length > 1 && (
                          <div className="ml-10 space-y-0.5 mb-1">
                            {Object.entries(details).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([nome, valor]) => (
                              <div key={nome} className="flex justify-between items-center px-4 py-0.5 text-xs text-muted-foreground">
                                <span>• {nome}</span>
